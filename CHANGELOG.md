...
- **Logos/Backdrops/Square Art each get their own subfolder** (`config/backups/{library}/logos/`, `/backdrops/`, `/square_art/`) — deliberately NOT the same flat folder posters already use, both to avoid filename collisions (a poster and a logo for the same title would otherwise share an identical "Title (Year).ext" name) and so each type can be backed up/restored/deleted independently. **Posters keep their exact original, un-subfoldered location and existing-vs-legacy-path fallback logic, completely untouched** — this was a deliberate choice to avoid any risk of a real user's existing poster backup silently becoming "not found" after upgrading.
- **Downloads reuse this app's own existing `fetch_and_cache_logo()`/`fetch_and_cache_backdrop()`/`fetch_and_cache_square_art()` functions** (imported from `movies.py`, the same ones backing the Logos/Backdrops/Square Art library-browsing grids) rather than a second, parallel "fetch this asset from Plex" implementation — called with `force_refresh=True` so a backup always reflects what's currently in Plex. This has the same, deliberate side effect a library scan already has: it also refreshes this app's own live cache for that item.
- **Restores reuse the same PIL-normalize-before-upload pattern `plexsend.py`'s manual per-item sends already established** (`normalize_logo_for_plex()`/`normalize_backdrop_for_plex()` from `backend/api/save.py`) for logo/backdrop/square-art restores — never forwarding raw downloaded bytes straight back to Plex unnormalized (see CLAUDE.md Quirk #26 for why that was a real bug the first time this app tried it). Poster restore is intentionally left exactly as it already was — a direct byte-for-byte round trip of bytes that came from Plex's own `/thumb` in the first place, no re-encode needed.
- See CLAUDE.md Quirk #55 for the full design (folder layout, why posters were deliberately left alone, and the reused-function list).

## v1.6.107 (2026-09-23)
### New Features
- **Logos, Backdrops, and Square Art now support Date Added sorting and label filtering**, matching what Movies/TV Shows already offer. No backend changes needed — `GET /api/movies`/`GET /api/tv-shows` (the same bulk endpoints these three views already fetch from via `useArtLibraryCache`) already return `addedAt` and `labels` per item; the three views just weren't using those fields yet. `SquareArtView.vue`'s `fetchItems()` `mapFn` (which whitelists a field subset, unlike Logos/Backdrops which pass the raw response through) needed both fields added to its whitelist — the other two views got them for free.
- **Discord/Apprise notifications now say when a logo, backdrop, or square art was sent, not just posters.** Previously, sending one of these three asset types from its own editor modal (`LogoEditorModal.vue`/`BackdropEditorModal.vue`/`SquareArtModal.vue`, or the standalone "Send Logo to Plex" button) produced zero notification at all — only the main poster-send path (`plexsend.py`'s `api_plex_send()`, plus batch/webhook/auto-generate) ever called into the notification system. `send_discord_notification()`/`send_apprise_notification()`/the shared `_build_notification_embed()` helper all gained an `asset_type` parameter (default `"poster"`, so every existing call site — and its rendered notification — is completely unchanged); a non-poster asset type adds one new "Asset" field to the embed (or one new line to the plain-text Apprise body), using the same emoji this app's own UI already uses for each (🖼️ Logo, 🎞️ Backdrop, 🔳 Square Art). Wired into `api_plex_send_logo()`/`api_plex_send_backdrop()`/`api_plex_send_square_art()` in `plexsend.py`, reusing the existing "manual send" notification toggle (`discordNotifyManual`/`appriseNotifyManual`) rather than adding new settings — these are all manual, single-item sends, same as the existing manual poster send.
- **Deliberately scoped to the standalone/manual send endpoints only** — batch/webhook/auto-generate's own internal logo-send sub-step (the "Also send logos to Plex" option within normal poster processing) was left untouched, since that event already triggers the main poster notification; adding a second, separate notification for the same underlying action would double up rather than add information. If a logo/backdrop/square-art *batch* notification turns out to be wanted too, that's a separate, larger change (those code paths don't currently know how to report per-asset-type sub-results at all).

## v1.6.106 (2026-09-22)
### Bug Fixes
- **Fixed the "Poster Shift Y" slider silently losing its intended "shift off-frame, reveal a black border" behavior on the default (non-square) canvas — a regression from v1.6.98, not a longstanding limitation.** User-reported: changed the slider (confirmed via the browser Network tab that the correct value was reaching the backend), but the rendered poster never moved. Root cause traced via `git log`/`git show` on `backend/templates/universal.py` and cross-checked against CHANGELOG.md's own v1.6.98 entry (several same-day version bumps landed in one git commit, so the commit message alone names only the last of them — CHANGELOG.md's per-version entries are the authoritative source): v1.6.98 changed `_resize_cover()`'s shift math from a raw pixel paste-offset onto the black base canvas to moving the crop window within whatever vertical "slack" the cover-fit resize leaves over, everywhere — a real, correct fix for Square Art (where the old paste-offset approach revealed blank canvas instead of more of the source image) but an *unwanted* change for the normal poster editor, where the black-border-at-extreme-shift look is a deliberate, relied-on effect, not a bug. Since TMDb/Fanart posters are essentially always exactly 2:3, matching Simposter's default 2000x3000 canvas at zoom=1.0, the crop-window approach has zero slack to work with there anyway — so the v1.6.98 change silently killed the slider's effect entirely for the app's own default case, rather than merely changing its *style* the way it did for Square Art.
- **First attempt at this fix was wrong and was reverted at the user's direction**: an initial pass kept the v1.6.98 crop-window approach everywhere and just reserved a small minimum overscan so the shift always had *some* real image content to move into — this restored a visible effect, but not the effect the user actually wanted (revealing real content instead of a black border). Corrected once the user clarified: "the -50/50 range would allow the poster to shift OFF the frame and introduce the black border (which is what I want)."
- **Fix**: `build_base_poster()` (`universal.py`) and `render_with_overlay_cache()` (`rendering.py`) now branch on `options["canvas_mode"]`. Square Art (`canvas_mode == "square"`) keeps the v1.6.98 crop-window approach unchanged — verified unaffected by this fix. The default canvas now calls `_resize_cover()` with no shift (plain center-crop) and pastes the result onto the black base canvas at a raw pixel offset (`shift_px = int(poster_shift_y * canvas_h)`) — the exact pre-v1.6.98 behavior, restored. Verified numerically: at `shift_y=-0.5`/`+0.5` on the default canvas, a canvas-edge row now samples as pure black (the exposed base canvas), while the same shift values on a square canvas still never expose pure black (real content only, matching Square Art's own established behavior, unaffected).
- **This is the single shared `_resize_cover()` function** — used by both the live-preview/normal render path (`build_base_poster()` in `universal.py`) and the cached/batch-fast-path (`render_with_overlay_cache()` in `rendering.py`, which imports the same function rather than duplicating it) — so the same `canvas_mode` branch had to be added to both call sites to keep them in lockstep (Quirk #19's established pattern), not just one.
- See CLAUDE.md Quirk #53 for the full mechanism, including the false-start lesson.

## v1.6.105 (2026-09-21)
### New Features
- **Added quick external links (TMDb, TVDB, Fanart.tv, MediUX) under the title in the manual editor** — movies, TV shows, and collections — matching what PosterTools shows for the same purpose: a fast way to jump to the source page for an item's metadata/artwork instead of searching manually. New shared `frontend/src/components/editor/ExternalLinksRow.vue`, used by both `EditorPane.vue` and `TvShowEditorPane.vue`.
- **URL schemes per media type** (verified live against real IDs before shipping, not assumed): TMDb (`themoviedb.org/movie|tv|collection/{tmdb_id}`) and TVDB (`thetvdb.com/dereferrer/series/{tvdb_id}`, TV only) were confirmed by fetching real pages (Breaking Bad, The Matrix). Fanart.tv (`fanart.tv/movie|series/{tmdb_id|tvdb_id}/`) couldn't be verified the same way — it returns a 403 to automated fetches — but matches the long-established convention other Plex poster tools use, and the id kind per media type (tmdb for movies, tvdb for TV) matches what `fanart_client.py`'s own API calls already use internally. MediUX (`mediux.pro/movies|shows/{tmdb_id}`) was confirmed live for both media types.
- **Collections only get a TMDb link** — Fanart.tv has no collection-level API resource at all (see CLAUDE.md Quirk #21), and TVDB/MediUX have no verified collection URL scheme, so those three are simply omitted rather than guessed at for that media type.

## v1.6.104 (2026-09-21)
### Improvements
- **An item with genuinely no poster art anywhere (TMDb/Fanart) — most commonly an unreleased movie/show TMDb has a page for but hasn't gotten key art uploaded for yet — no longer generates a "failed" History entry on every retry cycle.** User asked why "The Beekeeper 2" (a real, correctly-matched movie — TMDb confirms it as an unreleased Jan 2027 title still missing poster/video data) kept appearing as `FAILED`/`AUTO` in History every 6 hours. Root cause: `_process_single_movie()`'s `raise Exception("No valid poster found (even after fallback).")` (and the TV-series equivalent, `_render_tv_series_poster()`'s `"No poster found for series"`) were caught by the same broad `except Exception` block used for genuine, unexpected errors — which unconditionally logs at ERROR level and writes a `db.record_poster_history(..., action="failed", status="failed", ...)` row. Functionally this isn't a failure at all — the source simply doesn't have art yet, and it may well succeed once TMDb/Fanart catches up, so the retry queue was already correctly leaving it pending (confirmed the v1.6.85 retry-forever-recurrence fix wasn't in play here, since the Plex item genuinely still exists) — the only real problem was the misleading History noise.
- **Fix**: both raise sites now use a new `NoPosterAvailableError` (`backend/api/batch.py`), caught by a dedicated `except` clause *before* the generic `except Exception` in both `_process_single_movie()` and `_process_single_tv_show()` — logs a plain `logger.info(...)` line ("No poster found for X (rating_key) — will retry at next interval") instead of `logger.error(...)`, skips the History write entirely, and returns `status: "no_poster_available"` (not `"error"`) so the retry queue still correctly treats it as needing another attempt later, without misfiring the existence-check logic that's specifically for confirmed-deleted items.
- **Found and fixed a real, separate bug in the same code path while tracing this through**: `backend/api/history.py`'s manual "Retry Now" button computed a TV show's `still_needs_retry` as `any(r.get("needs_retry") for r in sub_results)` with no fallback for a missing/empty `results` list — `any([])` is `False`, so any TV-show-level hard failure (this one included, but not limited to it — the pre-existing "No TMDb ID found" case had the identical exposure) would incorrectly mark the item "resolved" instead of leaving it pending. `backend/scheduler.py`'s automatic retry job already had the correct fallback (`else: still_needs_retry = True`) for exactly this reason; `history.py`'s manual retry path was just never brought in line with it. Fixed to match.
- **If you add another "no source data available yet" case anywhere in the render pipeline, raise `NoPosterAvailableError` (or a similarly-scoped new exception) rather than a bare `Exception`** — the generic handler's failure/History-write behavior is correct for genuinely unexpected errors and would misreport this class of "nothing's actually broken" case as a fail again.

## v1.6.103 (2026-09-20)
### New Features
- **Added a Cleanup tool** (Settings → Cleanup), modeled on Kometa-Team's ImageMaid but scoped entirely to Simposter's own disk cache and DB rows — never needs a second filesystem mount into Plex's server data directory the way ImageMaid itself does. **Scan** (dry-run, never modifies anything) reports orphaned poster/logo/backdrop/square-art cache (including the generated thumbnails added in v1.6.102), stale overlay effect renders, unused uploaded images, unused overlay badge assets, and old History entries — with a count and reclaimable size per category. **Clean Selected** moves matched candidates into a reversible trash (`config/cache/_cleanup_trash/`) rather than deleting anything outright; a separate **Cleanup Trash** section lists every batch with **Restore** and **Empty** actions, so nothing is ever permanently gone until you explicitly empty it.
- **Added one-click Plex Server Maintenance buttons** (Empty Trash, Clean Bundles, Optimize Database) in the same tab — plain calls to Plex's own maintenance API, verified against `python-plexapi`'s real source rather than guessed (`PUT /library/optimize?async=1`, `PUT /library/clean/bundles?async=1`, and `PUT /library/sections/{key}/emptyTrash` looped over every section, since Plex has no single server-wide empty-trash call).
- **Added an "ImageMaid Integration — Coming Soon" section** naming the one real gap neither the cache cleanup nor the Plex maintenance buttons close: pruning old, no-longer-selected poster/art versions sitting inside the metadata bundle of an item still active in your library (e.g. leftover images from past Kometa runs). "Clean Bundles" only removes a bundle entirely, and only for items gone from every library — it doesn't touch that. Closing it for real would require the same Plex-server-filesystem access ImageMaid itself needs, which this whole feature was deliberately built to avoid; noted here rather than silently left unaddressed.
- **Cleanup can now run on a schedule** (Settings → Cleanup → Scheduled Cleanup) — cron-based (same mechanism as the existing library-scan scheduler), default weekly (`0 3 * * 0`) if enabled, with a category checklist and a configurable History-age threshold. A scheduled run only ever scans and moves matches to the trash — it never auto-empties the trash, exactly like a manual run.
- **Global search now asks "what do you want to edit?"** — Poster, Logo, Backdrop, or Square Art — instead of always opening the full poster editor. Works from any page, since global search always has. Logo/Backdrop/Square Art route straight into the same per-item editor modals the Logos/Backdrops/Square Art library views already use; no new editor UI was built.

### Improvements
- **Existing installs upgrading into this version get Scheduled Cleanup turned on automatically; fresh installs don't.** A deliberate, one-off exception to this app's otherwise universal "every automation feature defaults off" rule — justified because an existing install has already had real time to accumulate genuine orphaned cache bloat by the time it upgrades, while a fresh install has nothing to clean yet. Detected via the same "is Plex already configured" signal this codebase already used once before for the `onboarding_completed` default, applied as a one-time, idempotent database seed — never re-applies, and never overwrites a choice you've since made yourself. Change or disable it anytime in Settings → Cleanup.

See CLAUDE.md Quirks #50 and #51 for the full design writeup, including what was verified against real data and real Plex API source before shipping.
...
- **The page-level "Refresh" button on Logos/Backdrops/Square Art now shows a busy state for the entire operation**, even when it's instant-loading from cache (the common case after the previous release's cache-first fix) — previously the busy flag only activated on a genuine cache miss, so clicking Refresh on an already-populated page gave zero visual feedback while it silently re-fetched in the background, inviting repeated clicks that could fire duplicate overlapping requests.

### Bug Fixes
- **Fixed Square Art's grid never updating after a successful Send to Plex, in any session.** Root cause was more fundamental than Backdrop/Logo's equivalent bug (fixed last release, CLAUDE.md Quirk #45): `SquareArtModal.vue` never declared an `updated` event on `defineEmits` at all, so there was nothing for `SquareArtView.vue` to listen for even if it had tried — a send only ever updated the modal's own local "Current Square Art" display. Added the missing emit and a new `onSquareArtUpdated()` handler wired up in `SquareArtView.vue`.
- **Fixed Logos/Backdrops/Square Art showing a stale image again after navigating back within the same session, even immediately after a successful send.** The sessionStorage cache added for instant-load in v1.6.100 is only ever written by a real list re-fetch — none of the three views' "item was updated" handlers touched it, so a send would correctly update the *live* in-memory grid but leave the *persisted* cache on the pre-send value, ready to repaint stale on the next cache-first visit. New `updateItem(key, patch)` on `useArtLibraryCache` patches an item and immediately re-persists to the same cache entry `fetchItems()` last populated; all three views' update handlers now go through it instead of mutating `items.value` directly.
- **Fixed a real, separate backend gap found while adding the backdrop per-card refresh above**: `api_backdrop()`'s `force_refresh` path never wrote the refreshed `art_url` back to `movie_cache`/`tv_cache` at all — the exact same "live re-check without sending" DB-write gap already fixed for Square Art's equivalent endpoint back in v1.6.99 (see CLAUDE.md Quirk #44), just never backported to Backdrop. Added the same `is_tv` param + DB-write-on-success logic, matching Square Art's endpoint shape.
- Investigated a report of the "Refresh" button "taking forever and slowing down other pages" — `GET /api/movies`/`/api/tv-shows` (what it actually calls) is a synchronous DB-only read with no Plex/TMDb calls, identical to what Movies/TV Shows/Logos already use, so no Backdrop-specific slow path was found in that request itself. The busy-state fix above addresses the one real related gap found (no feedback → repeated clicks → duplicate requests); if slowness persists, it's more likely a library *scan* (a genuinely Plex-heavy, 4-way-per-item operation) being conflated with this page's own Refresh button, which never triggers one.

See the new CLAUDE.md Quirk #48 for the full root-cause writeup.

## v1.6.100 (2026-09-19)
### Improvements
- **Logos, Backdrops, and Square Art now paginate, matching Movies/TV Shows.** Previously all three fetched-and-rendered every item in the library on every page load, no matter how large — a real production log showed a single page view of Backdrops firing enough `/api/backdrop/{key}` requests to blow straight through the rate limiter. New `frontend/src/composables/usePagedItems.ts` (`usePagedItems(sourceItems)`) provides `page`/`totalPages`/`pagedItems`/`nextPage`/`prevPage`/`resetPage`, sized from the same `settings.posterDensity` ("Poster Density," Settings → General) Movies/TV Shows already use — so page size stays consistent across every library view instead of each one picking its own number. `LogosView.vue`/`BackdropsView.vue`/`SquareArtView.vue` all now render `pagedItems` instead of the full filtered/sorted list, with a Prev/Next pagination bar below the grid (page resets to 1 on any filter/search/sort change).
- **Logos, Backdrops, and Square Art now load instantly on repeat visits, same as Movies/TV Shows** — the sessionStorage cache-first pattern added in the previous release (`useArtLibraryCache`) combined with pagination means a typical revisit now renders from cache immediately and only fetches a page's worth of art, instead of a full cold list fetch plus every item's art request at once.

### Bug Fixes
- **Fixed "Rate limit exceeded" (429) errors when browsing Backdrops or Square Art**, reported directly from a production log showing `/api/backdrop` hitting `300/300 requests in 60s` and every request past that failing. `/api/backdrop` and `/api/square-art` (`backend/middleware/rate_limit.py`) had no dedicated `endpoint_limits` entry — the same gap `/api/movie`/`/api/tv-show`/`/api/logo` already had fixed for them back in v1.6.63 (see CLAUDE.md Quirk #26), just never extended to these two newer endpoints when they were added. Both are the same trust class (cheap per-item disk-cache reads, not live external API calls) as Logo, so both now get the same 1500/60s limit. Combined with the pagination fix above — which removes the underlying full-library request burst that triggered this in the first place — but fixed independently, since a large-enough single page (or the "In Plex"/"Missing" filters, which don't reduce the per-item art-fetch count) could still exceed the old default on its own.
- **Fixed the "Current Backdrop"/"Current Logo" preview in `BackdropEditorModal.vue`/`LogoEditorModal.vue` not updating after a successful Send to Plex** — both modals read `props.item` directly, a snapshot object taken when the modal opened and structurally distinct from the grid's own copy of that item (which a successful send *does* update, via its `updated` emit). Fixed with a local override ref (matching the pattern `SquareArtModal.vue` already used) that takes priority over the stale prop once a send succeeds.
- Fixed a Docker build failure (`vue-tsc --build` type-check errors in `SquareArtModal.vue`, both real `T | undefined` array-index-access cases under `noUncheckedIndexedAccess`) that had been passing local `vue-tsc --noEmit` checks — that command is a materially different check from this project's actual `npm run build`/`"type-check"` script (`vue-tsc --build`, project-reference/incremental mode). See the new CLAUDE.md Quirk #46 for the verification-method lesson this left behind.

## v1.6.99 (2026-09-19)
### New Features
- **Square Art now shows "Current Square Art (in Plex)"** when you open a title — user-reported: "the current square art posters in simposter dont match whats in plex," since the modal only ever generated a fresh candidate-based preview with no way to see what was actually already active in Plex. New `square_art_url` column on `movie_cache`/`tv_cache` (mirroring `art_url`/`logo_url` exactly through every read/write path this project already has established for that shape), a new `GET /api/square-art/{rating_key}` serve endpoint, and a third `make_art_cache()` instance (`SQUARE_ART_CACHE_DIR`, `direct_endpoint="squareArt"`, image type `backgroundSquare`) reusing the exact factory built for Backdrops.
- **Deliberately not populated by library scans** (unlike Logos/Backdrops) — square art is a niche, manually-triggered asset that most items will never have, so adding a 4th parallel Plex request per item to every scan wasn't worth it. Instead: (1) `POST /api/plex/send-square-art` now caches the just-uploaded bytes locally and returns the new `square_art_url` directly, so `SquareArtModal.vue` can show what was just sent without re-fetching from Plex (which might not have finished processing the upload yet — the same race backdrop's post-upload caching already exists to avoid); (2) the modal's default "Current Square Art" load always falls through to a genuine live Plex fetch the first time (no local cache existing yet is exactly when `fetch_and_cache()`'s `force_refresh=False` path still hits Plex), so square art set by another tool is picked up correctly too; (3) an explicit refresh button re-checks Plex on demand for anything that might have changed since.
- Verified against mocked Plex responses: the direct `/squareArt` GET convenience path is tried first (matching the same pattern that proved necessary for Backdrops), a second call reuses the local cache without hitting Plex again, and `square_art_url` survives the full DB round-trip and the `Movie` response-model serialization (the same silent-field-stripping trap documented for `art_url`).
...
### Bug Fixes
- **Fixed the "Poster Position (Up/Down)" slider not actually revealing more of the source image.** `_resize_cover()` (`backend/templates/universal.py`) cropped the poster down to *exactly* the target canvas size first, then a separate step pasted that already-fixed-size image at a vertical offset — with zero slack left after the initial crop, any shift just clipped the poster's own edge and revealed empty canvas on the opposite side, never more of the actual source image. The shift is now applied *during* the cover-crop itself, moving the crop window within whatever vertical slack the cover-resize actually left over — substantial for a 2:3 poster going into a 1:1 square canvas, which is why this was so visible in Square Art specifically, though the identical bug affected the normal editor's own "Poster Shift Y %" slider too (just less noticeably, since a 2:3 poster on the default 2:3 canvas has little slack to begin with). Fixed in both call sites (`build_base_poster()` and `rendering.py`'s overlay-cache fast path, which duplicates the same logic). Verified with a row-fingerprinted gradient test image confirming a full-range shift reaches the source's actual top/bottom edge, and that `shift=0` is mathematically identical to the prior behavior (no regression for existing renders).

## v1.6.97 (2026-09-19)
### Improvements
- **Added a "Poster Position (Up/Down)" slider to Square Art** (`SquareArtModal.vue`) — no new backend logic needed, since this reuses the exact same `poster_shift_y` option (`-0.5..0.5`, already consumed by `build_base_poster()`) the normal editor's "Poster Shift Y %" slider has always driven, just exposed in Square Art's UI too. A square crop discards proportionally much more of a source poster's height than the normal 2:3 canvas does, so recentering which part of the image survives the crop matters more here — this is the direct follow-up to the "some posters don't work well with the ratios" feedback that also led to the v1.6.96 candidate-picker rebuild. Re-seeds from the selected preset's own saved value on template/preset change, then freely adjustable; debounced (400ms) the same way `EditorPane.vue` debounces its own slider-driven re-renders. Verified end-to-end with a real pixel-diff render test confirming the option actually moves content on a 2000×2000 canvas, not just that it's wired up.

## v1.6.96 (2026-09-19)
### Bug Fixes
- **Square Art was compositing over an already-composited image — fixed with a real poster/logo picker.** User caught this from a screenshot: v1.6.93's Square Art used `item.poster` (the grid's current-Plex-poster field) as the render background. For a Simposter-managed library, that "current poster" is almost always *already a Simposter-rendered poster* — a photographic background with a logo already baked in by a prior normal render. Squashing that into a square canvas and then compositing *another* logo (`item.logo_url`) on top of it produced a broken double-composited result, not just a badly-cropped one — which is what the earlier "some posters don't work well with the ratios" report was actually describing.
- **Fix**: `SquareArtModal.vue` now fetches raw TMDb/Fanart poster and logo candidates for the title (`GET /api/tmdb/{id}/images` or the TV equivalent — the exact same source `EditorPane.vue`'s background always uses, never Plex's current poster) and shows them as clickable galleries, mirroring `LogoEditorModal.vue`'s candidate-grid pattern (including its TMDb-thumbnail-404 fallback handling). Defaults to a textless poster candidate when one exists, and the first available logo (or none) — both fully overridable by clicking a different tile. Since TMDb/Fanart URLs are already public, absolute URLs, this also removes the need for the internal-cache-URL-prefixing dance the old `item.poster`/`item.logo_url` reuse required.
- `SquareArtView.vue`'s browsing grid (pick-a-title) is unaffected — it still shows the current Plex poster purely as a "which movie is this" visual aid when choosing what to open, which was never the bug; only what got fed into the actual render was wrong.

## v1.6.95 (2026-09-19)
### Bug Fixes
- **"Add to Retry Queue" UI polish from direct user testing**: moved the button out of the Save/Send row and placed it below "Current Plex Logo" (`preview-existing` sidebar) in both `EditorPane.vue` and `TvShowEditorPane.vue` — visually separated from the save/send actions since it's a genuinely different kind of action. `AddToRetryQueueModal.vue`'s template dropdown now excludes `kometa` (a `EXCLUDED_TEMPLATES` set) since it's a collections-only template (Quirk #21) and retry-queue items are never collections. `SquareArtModal.vue` gets the identical exclusion for the same reason.
- **Backdrop uploads to Plex were failing 100% of the time** with a dropped SSL connection (`SSLEOFError`), reproduced twice by the user against a real server. Root-caused by pattern-matching this codebase's own already-working poster/logo uploads: both use a **pluralized** write endpoint (`/posters`, `/clearLogos`) even though the read-side convenience path is singular (`/thumb`) — v1.6.94's backdrop fix confirmed the singular `/art` GET works for *reading* the current backdrop, but the matching *write* path was never actually verified and was almost certainly supposed to be `/arts` (plural) to match the established convention. `api_plex_send_backdrop()` (`backend/api/plexsend.py`) now tries `/arts` first, falling back to `/art` if that fails, logging which one actually succeeds — resilient to either being wrong, and a fresh connection attempt on the fallback also sidesteps a possibly-poisoned pooled connection regardless of which path is correct.

## v1.6.94 (2026-09-19)
### Bug Fixes
- **"Add to Retry Queue" didn't let you pick a different template — fixed with a real picker.** User feedback: the whole point of queuing an item is "I'm sending a text poster right now, but I want it automatically switched to my textless+logo template once a textless poster exists" — the v1.6.91 button instead silently reused whatever template/preset was *currently selected in the editor* (which, in that exact scenario, is the non-ideal one). New `AddToRetryQueueModal.vue` opens on click, loads all templates/presets (`GET /api/presets`), pre-fills with the editor's current selection as a starting point but lets the user pick a genuinely different one before confirming. No backend change — still the same `POST /api/retry-queue` endpoint from v1.6.91.
- **Backdrops never actually cached during a scan, despite the scan reporting it fetched them.** Root cause: `fetch_and_cache_backdrop()` (via `make_art_cache()`, `backend/api/art_cache.py`) looked for a `type == "art"` entry inside Plex's `/library/metadata/{id}` JSON response's `Image[]` array — which, per a real user log, apparently doesn't reliably include a background-art entry at all (the array only reliably has `clearLogo`, which is why Logos worked and Backdrops silently didn't). Fixed by adding a `direct_endpoint` option to `make_art_cache()`: for backdrops, it now tries `GET {PLEX_URL}/library/metadata/{rating_key}/art` **first** — the same direct-fetch convention `fetch_and_cache_poster()` already uses via `/thumb`, and the exact path `plexsend.py`'s upload already POSTs to successfully — falling back to the old `Image[]` lookup only if that fails. Verified via mocked-request tests: both the direct-endpoint success path and the Image[]-fallback path behave correctly.
- **Square Art's preview failed with `"Private/internal network URLs are not allowed for this host"` whenever Simposter is reached via a LAN IP (not localhost).** `_SAFE_INTERNAL_PATH_PREFIXES` (`backend/middleware/validation.py`, the SSRF allowlist for this app's own local-disk-served endpoints) covered `/api/movie/` and `/api/tv-show/`, but not the sibling `/api/logo/` or the new `/api/backdrop/` — both top-level routes, not nested under `/api/movie/`. Every prior feature only ever sent a `/api/movie/.../poster`-shaped URL through the render pipeline's `background_url`; Square Art is the first to also reuse a title's cached **logo** (`/api/logo/{key}`) this way, which is what exposed the gap. Added both missing prefixes — verified via `check_ssrf_safe()` directly.
- A backdrop upload failure in the same user report (`SSLEOFError` against a `plex.direct` remote-access hostname) was traced to a transient network/TLS drop during the upload itself, not a code bug — `POST /api/plex/send-backdrop` uses the identical `plex_session`/`timeout=20` pattern already used for posters and logos. No code change; flagged here in case it recurs specifically for backdrop-sized uploads over a slow/remote connection.

## v1.6.93 (2026-09-19)
### New Features
- **Added a "🔳 Square Art" library section** (movies and TV shows) — generates square (1:1) art from a title's existing template/preset, rather than a new template. New `options["canvas_mode"] = "square"` render-time flag (`backend/templates/canvas.py`, never persisted onto a preset) — chosen over a new `template_id` since a new id would require touching the renderer registry, multiple `if template_id ==` dispatch branches, and ~13 frontend literal-comparison sites, plus fragmenting presets into a separate namespace the user would have to hand-sync with their normal poster preset.
- **Canvas width stays identical (2000px) across every mode — only height changes**, which is what keeps this a small, surgical change: `resolve_canvas_size(options)` now backs every previously-hardcoded `2000x3000` site (`build_base_poster()` in `universal.py`, the kometa flat-color background synthesis and `render_with_overlay_cache()`'s fast path in `rendering.py`), and because canvas width never changes, the large majority of existing options (matte/fade/vignette ratios, `uniform_logo_offset_x/y`, `uniform_logo_max_w`, `kometa_logo_width`, badge positions) stay correct unmodified. Only two height-anchored absolute-pixel fields needed rescaling for a non-default canvas height — `uniform_logo_max_h` and `kometa_logo_offset_y` — handled by a small `scale_options_for_canvas()` helper called at the top of both template renderers.
- **New `SquareArtView.vue`/`SquareArtModal.vue`** — pick a title, choose a template/preset, live-preview at the square canvas size, Save to Disk (a separate `square-art.jpg` filename so it never overwrites the title's normal saved poster, using `SaveRequest`'s existing `filename` override). No Send to Plex — Plex has no upload slot for a square-aspect asset, called out explicitly in the UI rather than left looking unfinished.
- Verified with real (non-network) render tests: both `uniformlogo` and `kometa` templates produce a correct 2000×2000 canvas end to end with `canvas_mode=square`, an unmodified 2000×3000 canvas otherwise, and `uniform_logo_max_h`/`kometa_logo_offset_y` rescale proportionally (e.g. 240px → 160px at the 2:3 canvas's default height ratio) while width-anchored fields are left untouched.

## v1.6.92 (2026-09-19)
### New Features
- **Added a "🎞️ Backdrops" library section** (movies and TV shows), mirroring the existing Logos tab's architecture: reuses the existing `GET /api/movies`/`GET /api/tv-shows` list endpoints (no new list endpoint), a per-item disk cache (`config/cache/backdrops/`), a `GET /api/backdrop/{rating_key}` serve route, and a `POST /api/plex/send-backdrop` upload route (Plex's `/art` endpoint — the backdrop counterpart to `/posters` and `/clearLogos`). New `BackdropsView.vue`/`BackdropEditorModal.vue` components (near-copies of `LogosView.vue`/`LogoEditorModal.vue`), sourcing candidate backdrops from the `backdrops` array `GET /api/tmdb/{id}/images` already returns (previously used only as a poster-template background source, not exposed as a Plex-syncable asset).
- **Extracted a shared `make_art_cache()` factory** (`backend/api/art_cache.py`) for "single-image-per-item Plex asset synced via the metadata Image[] array" — the Logo cache helpers (`_logo_cache_path`/`_logo_cache_url`/`_save_logo_cache`/`fetch_and_cache_logo`, previously bespoke in `backend/api/movies.py`) now come from the same factory as the new backdrop ones, rather than duplicating that logic a second time. New `normalize_backdrop_for_plex()` (`backend/api/save.py`) reuses `encode_poster_for_plex()`'s RGB-flatten + tiered-PNG-then-JPEG-fallback logic (backdrops are opaque, multi-MB photographic images, unlike small transparent logos, so `normalize_logo_for_plex()`'s always-PNG approach was the wrong fit).
- **New `art_url` column** on `movie_cache`/`tv_cache` (`backend/database.py`), threaded through every existing read/write path that already handles `logo_url` (`upsert_movie_cache`/`upsert_tv_cache`, `bulk_refresh_cache`/`bulk_refresh_tv_cache`, `get_cached_movies`/`get_cached_tv_shows`, new `update_movie_art_url`/`update_tv_art_url`) and populated during every library scan alongside the existing parallel poster/logo fetch. `schemas.py`'s `Movie` model also gained `art_url` — required since `GET /api/movies` is declared `response_model=List[Movie]`, which silently strips any field not on that class regardless of what the underlying dict contains.

## v1.6.91 (2026-09-19)
### New Features
- **Added "Add to Retry Queue" to the manual editor** (movies and TV shows). Motivating case: no textless poster exists yet for a title, so you manually pick a different poster right now to get something live, but want Simposter to automatically pick up a proper textless render later once TMDb/Fanart actually has one — using whatever template/preset is currently selected in the editor. New `POST /api/retry-queue` endpoint (`backend/api/history.py`) wraps the existing `db.add_to_retry_queue()` upsert with a new `manual_textless_pending` reason (`RETRY_REASON_MANUAL_TEXTLESS`, `backend/database.py`) — no schema change needed, the table's `reason` column was already free text.
- **Closed a real gap in the existing retry-resolution logic while wiring this up**: `needs_retry` only ever flagged a missing textless poster when the preset's `fallbackPosterAction` was `"template"` — the far more common `"continue"` case (the effective default whenever unset) silently fell through, meaning a manually-queued "waiting for textless" item could have been marked resolved on its very first retry pass even though nothing had changed. Fixed with a new opt-in `require_textless_poster` flag (`MovieBatchRequest`/`TVShowBatchRequest`, `backend/schemas.py`) that forces `needs_retry=True` until the actually-selected poster has `has_text=False`, independent of `fallbackPosterAction` — threaded through `_process_single_movie`/`_render_and_save_poster` (`backend/api/batch.py`) and both existing resolvers (`scheduler.py`'s scheduled retry job, `history.py`'s manual "Retry Now" button), each deriving the flag from the retry-queue row's own `reason`. Additive only — defaults `False` everywhere except this one path, so existing retry behavior for the auto-generated reasons (`no_logo`, `poster_fallback`, etc.) is unchanged.

## v1.6.90 (2026-09-19)
### Bug Fixes
- **Fixed retry queue items appearing twice after a delete-then-re-add in Plex.** User-reported and confirmed via a History screenshot: several titles (e.g. "Star Wars: Starfighter," "Shrek 5," "The Beekeeper 2," "A Quiet Place Part III," "Jumanji: Open World") each showed both a `PENDING` row and an `ABANDONED` row for the same movie. Root cause: `resolve_retry_queue_item(rating_key, "abandoned")` (`backend/scheduler.py`) only ever updated the row's `status` column — it never deleted it, despite its own log line claiming "removed from retry queue." A movie/show whose Plex item is deleted then later re-added gets a brand-new `rating_key` (same mechanism as Quirk #41's poster-reuse cache), so the old, now-stale abandoned row for the old `rating_key` just sat there permanently next to the fresh pending row created for the new one.
- **Fix**: all three `resolve_retry_queue_item(rating_key, "abandoned")` call sites in `backend/scheduler.py`'s `_run_poster_retry()` now call `db.remove_from_retry_queue(rating_key)` instead — an abandoned item is deleted outright, matching what the log message already claimed was happening. `get_retry_queue()` (`backend/database.py`) no longer needs to filter for `'abandoned'` status in its default view, and a one-time startup cleanup (`DELETE FROM poster_retry_queue WHERE status = 'abandoned'`, idempotent, runs every `init_database()`) sweeps out any stale abandoned rows already sitting in an existing install's database from before this fix — including the exact rows from the reporting user's screenshot.

## v1.6.89 (2026-09-04)
### Bug Fixes
- **Corrected the age model for v1.6.88's "Reuse Cached Poster For (days)" feature.** The initial implementation tracked staleness via the cached poster file's own mtime — which really means "days since this poster was last (re)rendered," not "days since this title was last confirmed present in the library." The user caught the flaw directly: a movie that's sat untouched (no reason to re-render it) for a year would incorrectly lose reuse eligibility purely from disuse, even though it never actually left the library — the opposite of the intended protection. As put by the user: *"the cache should have it and then it should be removed from cached folder after x-days... the x-days gives a grace period of saying 'oh its been more than x-days, so obviously that item has been removed for good.'"*
- **Fix**: a new DB table, `tmdb_last_seen(media_type, tmdb_id, season_index, last_seen_at)` (`backend/database.py`), tracks the last time each TMDb-identified title was seen in a library scan, fully decoupled from rendering/sending. `season_index` defaults to `-1` rather than `NULL` in the composite primary key — SQLite treats every `NULL` as distinct for uniqueness purposes, which would have let duplicate rows silently accumulate instead of one row correctly upserting. `api_scan_library()` (`backend/api/movies.py`) touches this table for every currently-present movie/show with a known `tmdb_id` after each library refresh; `load_render_cache_by_tmdb()` (`backend/config.py`) now checks days-since-last-seen from this table instead of file mtime, and a purge sweep (`purge_stale_render_cache_by_tmdb()`, run once per scan) actually deletes both the DB row and the on-disk cache file once the grace period fully elapses, rather than merely no longer serving it.
- Season-level reuse remains conservative on purpose: the scan only records a series-level "last seen" (not per-season), so a specific season's cache always falls back to a fresh render unless/until per-season tracking is added — erring toward correctness over completeness given this project's prior experience with the TV rendering path's fragility (see CLAUDE.md Quirk #13).

## v1.6.88 (2026-09-04)
### New Features
- **Added a "Reuse Cached Poster For (days)" setting (Settings → Automation, `reuseCachedPosterDays`, 0 = disabled by default).** User-reported: a batch of movies with manually-tuned posters got auto-regenerated at 2:55 AM, overwriting the existing work. Root-caused directly from the production log (`config/logs/simposter.log`): a scheduled scan found 25 movies with `rating_key`s Simposter's own cache had never seen before (`[SCAN] Cached 645 movies for library 1 (25 new)`), and — because Auto-Generate was enabled — treated all 25 as genuinely new library additions. Every one had zero pre-existing Plex labels (`Add-label ... kept 0 existing`), ruling out a Simposter reprocessing bug; the actual cause (confirmed by the user) was a tool re-grabbing/re-downloading files (UMTK re-fetching trailers, though the same thing happens with a plain Radarr/Sonarr upgrade), which can cause Plex to re-match an item under a brand-new internal `rating_key` — indistinguishable, from Simposter's rating_key-keyed cache's point of view, from a movie it's never touched before.
- **How it works**: a new disk cache (`config/cache/poster_renders_by_tmdb/`, `backend/config.py`'s `save_render_cache_by_tmdb()`/`load_render_cache_by_tmdb()`) tracks the most recently sent poster keyed by `(media_type, tmdb_id, season_index)` instead of `rating_key` — populated on **every** successful Plex send (manual, batch, webhook, auto-generate alike, so a manually-tuned poster is exactly as reusable as an auto-generated one), using file mtime for age tracking rather than a new DB table. When a webhook or scheduled scan encounters a `rating_key` it's never cached before, and this setting is > 0, it checks this cache first: if a poster for the same TMDb ID (and season, for TV) was sent within the configured window, that poster is resent to the new `rating_key` instead of a fresh TMDb/Fanart fetch + render + upload.
- **Deliberately scoped to `source in ("webhook", "auto_generate")` only** — the two paths that passively *discover* items (a scan noticing an unfamiliar rating_key, or an import webhook) rather than a user explicitly choosing to (re)generate something via manual batch/save/send, which must always produce a fresh render regardless of this setting. New helper functions `_resend_cached_bytes_for_movie()`/`_resend_cached_bytes_for_tv()` (`backend/api/batch.py`) handle the actual resend (upload, label remove/add, history, cache refresh), mirroring the shape of the existing resend-fast-paths already in `auto_generate.py`/`webhooks.py` rather than routing through the full render pipeline — there's nothing to render, the bytes already exist.
- **TV note**: the reuse check sits inside the shared `_render_and_save_poster()` (used for both series and season posters), which only receives already-resolved `poster_url`/`logo_url` from its caller — so this skips the render + upload for a hit, but not the TMDb/Fanart fetch (which already happened one level up before this function was even invoked). Still meaningfully faster and, more importantly, correctly avoids overwriting a season poster whose show got a new rating_key.

## v1.6.87 (2026-09-02)
### Improvements
- **Added PNG app icons alongside the SVG favicon.** User provided rendered PNGs (192px, 512px, and a 180px Apple touch icon variant) of the fanned-poster-cards logo; added as `frontend/public/icon-192.png`, `icon-512.png`, `apple-touch-icon-180.png`, referenced from `index.html` via `<link rel="icon" type="image/png" sizes="...">` (both sizes, ahead of the existing `favicon.ico` fallback) and `<link rel="apple-touch-icon">`. Gives browsers/contexts that don't reliably render an SVG favicon (see Quirk #39/#40's favicon saga) a proper raster fallback, and gives mobile "Add to Home Screen" a real icon instead of a screenshot of the page.
- **The Apple touch icon needed an opaque background, unlike the other icons.** The user's source PNG had the same transparent background as the SVG/other icons (intentional there — see Quirk #40's light/dark theme work), but iOS doesn't apply any background behind an apple-touch-icon itself, so a transparent one typically renders oddly (commonly as solid black) on a home screen. Composited it onto the icon's own dark navy (`#0F172A`) background via Pillow before saving — pure raster compositing, no SVG rasterization involved, so this didn't hit the `cairosvg`/native-`cairo`-library gap noted elsewhere in this project. The 192px/512px PNG icons and the SVG favicon are unchanged (transparent is correct for those, since browsers/OSes do provide their own background behind a regular favicon).
- **Also added `.github/icon-512.png`** (same source image) as a plain repo asset — not wired into anything automatically, just a ready-to-grab file for manually uploading as the GitHub repo's social-preview image (Settings → General → Social preview), which GitHub only accepts as a raster upload through its own UI, not a file reference in the repo.
