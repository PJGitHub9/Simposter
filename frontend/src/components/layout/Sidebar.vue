...
const getIcon = (key: TabKey | string) => {
  const icons: Record<string, string> = {
    ...
    'tv-shows': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="15" rx="2" ry="2"/><polyline points="17 2 12 7 7 2"/></svg>`,
    ...
    'backup-restore': `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9M3 12l4-3M3 12l4 3"/></svg>`,
  ...
  if (key.startsWith('batch-') || key.startsWith('tv-batch-')) return icons['batch-edit']
  if (key.startsWith('collections-')) return icons.collections
  if (key.startsWith('assets-') || key.startsWith('tv-assets-')) return icons['local-assets']
  if (key.startsWith('backup-') || key.startsWith('tv-backup-')) return icons['backup-restore']
  return ''
}
</script>
